  ...
  string postJSON(
    ...
    try {
      if (simModem.isGprsConnected()) {
        ...
        // Before fetching the response status code, check if client connected successfully
        if(!httpClient.connected()) {
          throw std::runtime_error("HTTP client not connected!");
        }
        ...
        if (this->responseOK(statusCode)) {
          ...
          throw std::runtime_error("Non-200 response received!");
        }
      } else {
        throw std::runtime_error("GPRS not connected!");
      }
    } catch (const std::runtime_error& e) {
      log_e("Exception in postJSON: %s", e.what());
    } 
    return "";
  }
  ...
  string print(ModemDriver &simModem, HttpClientDriver &httpClient, Headers &headers, const char * endPoint, string requestBody) {
    if (simModem.isGprsConnected()) { 
      ...
      if (this->responseOK(statusCode)) {
        ...
        log_w("Throwing None 200 response");
        throw 1;
      }
    } else { 
      log_w("Throwing GPRS disconnected");  
      throw 0;
    }    
  }
  ...
  bool download(
    ...
    if (simModem.isGprsConnected()) {
      ...
      if (this->responseOK(statusCode)) {
        ...
        log_d("%d bytes downloaded and written to file : %s", readLength, file.name());
        file.close();
        log_d("Actually read:  %u", readLength);
        log_d("Duration:       %us", millis() - timeoutStart);
        log_d("Wait for 3 seconds");
        vTaskDelay(3000);
        ...
        log_w("None 200 response in https download");
      }
    } else {
      log_w("Modem not connected in https download");
    }
    return false;
  }
  ...
  void readHeaders(HttpDriver &httpClient) {
    ...
    try {
      ...
      while (httpClient.headerAvailable() && millis() - headerTimeout < this->_httpsTimeout) {
        ...
        log_d("%s : %s", headerName.c_str(), headerValue.c_str());
      }
      int length = httpClient.contentLength();

      if (length >= 0) {
        log_d("Content length: %i", length);
      }
    } catch(uint8_t error) {
      throw 2;
